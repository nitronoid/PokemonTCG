...
bool PokemonCard::canPlay(Game &io_game) const
{
  // If the pokemon is basic, we check for free slots
  if (!m_stage)
    return io_game.freeSlots(PTCG::PLAYER::SELF).size();
  // Otherwise we check that the pre-evolution of it is in play
  else
    return io_game.filterSlots(PTCG::PLAYER::SELF, [name = m_preEvolution](const auto _slot)
    {
